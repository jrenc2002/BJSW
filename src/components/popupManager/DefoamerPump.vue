<template>
  <div class="h-[94vh] transition-all duration-300 ease-in-out shadow bg-white rounded-2xl" :class="[AppGlobal.isDrawerState? 'w-[calc(94vw-15rem)]':'w-[94vw]']">
    ...
    <div class="h-[4%] self-stretch justify-start items-center  inline-flex mt-3  w-full ">
      <div class="w-[calc(10rem)] text-xl leading-10 text-zinc-900 text-2xl font-medium leading-loose left-4 relative">补料泵1控制</div>
...
const initTableData = () => {
  ...
  const deviceProperties = [
      ...
      {name: '累计补料量 ml', prop: 'feed0_pump'},  // 这里选择了补料关联氧含量标志位，因为它看起来是一个功能关联标志
      {name: '补料速度 ml/h', prop: 'feed0_pump_now_set_speed'}  ]
  ...
  deviceProperties.map((deviceProp, deviceIndex) => {
    ...
    initheaderData.forEach((header, index) => {
      ...
      else if (deviceProp.prop == "feed0_pump") {
          if (DeviceManage.deviceList[index]?.nowData?.feedPumpSpeed !== null) {
    
              tableItem[header.props] = DeviceManage.deviceList[index]?.deviceSet?.defoamerPumpSumStepCount;
